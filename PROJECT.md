...
- **2026-08-23 (console pass):** Synced `ui-tweaks` to `origin/main` (fast-forward — main already contained the Quorum UI and had built Spider-Sense, the Bugle and Docker on top) then reworked five console surfaces.
  - **Flocks now lead the dashboard.** The commission form moved below them: what is already running is the reason to open the page; commissioning is the occasional act. The page gained a real `<h1>` and a targets/scrapers/signals tally.
  - **Commission header rebuilt as two columns** — a tight lede on the left, the three beats of the mechanic (three scrapers → one vote → zero humans) on the right, which replaces the wall of paragraph and the dead half of the strip.
  - **Form regrouped and a layout bug fixed.** Three fields sat in a two-column grid, so the third wrapped and left a ragged hole; and the `<legend>` was `float: left; width: 100%`, which collapsed the grid beside it to near-zero width. Now `role="group"` captions over three labelled groups, plus a schema table with real column headings so the bare `key` radios have something to belong to.
  - **Spider-Sense re-presented.** A full-bleed red slab reads as a crashed page rather than a detector that fired correctly, so severity moved to a rail and a chip, the evidence into a framed mono readout with a `was → now` pair, and the fleet-wide note into its own inset. "Nothing drifting" is now a stated result rather than an empty box.
  - **Daily Bugle:** masthead now carries the key to the one number it invents — the four weighted contributions with their weights — and the leaderboard gained rank numerals and a bigger score. The bar's `drift`/`dispute` pair were two ambers differing only in lightness; re-stepped to red / amber / indigo / teal (`--teal: #0a9ba1`), validated for CVD separation, with a 2px surface gap between segments. Methodology prose became a scannable definition list.
  - Presentational pieces extracted to `flock-card`, `commission-form` and `drift-signals` so they can be rendered against fixtures without a database.
